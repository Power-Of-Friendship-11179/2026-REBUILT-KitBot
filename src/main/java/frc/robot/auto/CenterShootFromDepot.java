 ...
 * of the hub.
 */
public class CenterShootFromDepot extends SequentialCommandGroup {
    private static final double BLUE_TO_DEPOT_HEADING = 135.0;
    private static final double DRIVE_TO_DEPOT_METERS = Inches.of(130.0).in(Meters);
    private static final double BLUE_FROM_DEPOT_HEADING = 153.0;
    private static final double DRIVE_FROM_DEPOT_METERS = Inches.of(-120.0).in(Meters);;
    ...
    public static final AutoSupplier getAutoSupplier(
        ...
        return new AutoSupplier(
                () -> new CenterShootFromDepot(driveSubsystem, ballSubsystem, shooterSubsystem),
                BLUE_POSE);
    }

    private CenterShootFromDepot(
            final CANDriveSubsystem driveSubsystem,
            final CANFuelSubsystem ballSubsystem,
            final CANShooter shooterSubsystem) {
        addCommands(
